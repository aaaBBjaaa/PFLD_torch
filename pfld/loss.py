...
class simplified_PFLDLoss(nn.Module):
    def __init__(self):
        super(simplified_PFLDLoss, self).__init__()

    def forward(self, attribute_gt, landmark_gt,
                landmarks, train_batchsize):
        ...
        weight_attribute = torch.sum(attributes_w_n.mul(mat_ratio), axis=1)
        l2_distant = 0.5 * torch.sum(
            (landmark_gt - landmarks) * (landmark_gt - landmarks), axis=1)
        wing = wing_loss(landmark_gt, landmarks)
        sm = smoothL1(landmark_gt, landmarks)
        return torch.mean(weight_attribute *
                          l2_distant), torch.mean(l2_distant)
...
def wing_loss(y_true, y_pred, w=10.0, epsilon=2.0, N_LANDMARK=106):
    ...
    losses = torch.where(w > absolute_x,
                         ...
                         absolute_x - c)
    losses = w * torch.log(1.0 + x / epsilon)
